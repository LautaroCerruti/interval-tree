...
ITree itree_insertar(ITree arbol, Intervalo * intervalo) {
  ...
  if (arbol) {                  // Si el arbol no esta vacio
    ...
    if (intervalo_extremo_izq(intervalo) ==
        intervalo_extremo_izq(arbol->intervalo)
    ...
    } else if (intervalo_extremo_izq(intervalo) <
               intervalo_extremo_izq(arbol->intervalo)
               || (intervalo_extremo_izq(intervalo) ==
                   intervalo_extremo_izq(arbol->intervalo)
                   && intervalo_extremo_der(intervalo) <
                   intervalo_extremo_der(arbol->intervalo)))
...
ITree itree_eliminar(ITree arbol, Intervalo * intervalo) {
  ...
  if (intervalo_extremo_izq(intervalo) ==
    ...
    if (!arbol->izq || !arbol->der) {   // Si el arbol tiene a lo sumo un hijo
      ...
      intervalo_destruir(arbol->intervalo);
      arbol->intervalo =
          intervalo_crear(intervalo_extremo_izq(nodo->intervalo),
                          intervalo_extremo_der(nodo->intervalo));
...
ITree itree_intersecar(ITree arbol, Intervalo * intervalo) {
  ...
  if (!arbol || !intervalo_valido(intervalo)
      || arbol->maxExtremoDer < intervalo_extremo_izq(intervalo))
    return NULL;
